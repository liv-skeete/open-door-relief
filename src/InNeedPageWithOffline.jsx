...
function InNeedPage() {
  ...
  const { isOnline, saveForOffline } = useOfflineSync();
  ...
  const submitRequest = async () => {
    ...
    try {
      ...
      if (isOnline) {
        ...
        await saveItem(STORES.REQUESTS, requestData);
        ...
        requestData.id = `local_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        
        // Save to offline storage
        const result = await saveForOffline('request', requestData);
        
        if (result.success) {
          alert(
            "You're currently offline. Your request has been saved locally and will be submitted automatically when you reconnect to the internet.",
          );
        } else {
          throw new Error("Failed to save request offline: " + (result.error || "Unknown error"));
        }
      }
    ...
    <div className="page">
      ...
      {!isOnline && (
        <div className="offline-warning">
          <strong>You're offline.</strong> You can still submit requests, which will be sent when you reconnect.
        </div>
      )}
      <div className="form">
      ...
      <div className="pledges-list">
        <h3>Available Pledges {!isOnline && <span className="offline-mode-indicator">Offline Mode</span>}</h3>
          ...
          {filteredPledges.length > 0 ? (
            filteredPledges.map((pledge, index) => (
              <div key={index} className="pledge-item">
                ...
                {pledge.id.startsWith('local_') && (
                  <div className="local-indicator">Saved locally</div>
                )}
              </div>
            ))
          ) : (
            <div className="no-pledges">
              <p>No matching pledges found.</p>
              {!isOnline && (
                <p>You're currently offline. Some pledges may not be available until you reconnect.</p>
              )}
            </div>
          )}
        </div>
